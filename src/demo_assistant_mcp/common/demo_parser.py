...
class DemoPrompt:
    ...
    def __post_init__(self):
        """Initialize variables set if not provided"""
...
def parse_demo_markdown(file_path: str) -> List[DemoPrompt]:
    ...
    Raises:
        ActionableError: If file not found, invalid format, or no prompts found
    """
    path = Path(file_path)
    ...
    if not prompts:
        raise ActionableError.empty_demo(file_path)
    ...
    for i, match in enumerate(header_matches):
        ...
        if not stripped_section.startswith('```'):
            raise ActionableError.invalid_format(
                file_path,
                f"prompt {i+1} is missing code block markers"
            )
        ...
        if not code_match:
            raise ActionableError.invalid_format(
                file_path,
                f"prompt {i+1} has malformed code block"
            )
